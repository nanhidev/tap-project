...
Scenario Outline: Schedule Panel Interview for a Candidate
  ...
  And a confirmation message should appear indicating the interview is scheduled
  And notifications should be sent to all selected participants
  And the scheduled interview should appear in the interview calendar
  And the interview details should be correctly displayed in the system
  And the candidate should receive an email notification with interview details
...
Scenario Outline: Schedule Panel Interview
  ...
  And the recruiter clicks on Edit
  And the recruiter changes the Date and Time of the interview
  And the recruiter adds or removes Attendees
...
Examples:
  | Date       | Time   | Attendees          |
  | 2023-10-01 | 10:00  | John Doe, Jane Doe |
  | 2023-10-02 | 11:00  | Alice Smith, Bob Lee|
